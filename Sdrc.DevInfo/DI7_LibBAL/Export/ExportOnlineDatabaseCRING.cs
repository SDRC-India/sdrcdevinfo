...
{
    internal class ExportOnlineDatabaseCRING
    {
        ...
        private bool ExportData(string dataNIds)
        {
            bool RetVal = false;
            ...
            try
            {
                ...
                this.InsertDataTableIntoTemp(dataNIds);
        ...
        private void InsertDataTableIntoTemp(string dataNIds)
        {
     ...
     " FROM UT_dataexport AS D, " + this.SourceTableNames.Indicator + " AS I, " + this.SourceTableNames.Unit + " AS U, " +
                ...
                //Get SubgroupVal Table
                DataTable DTSubgroupVal = this.SourceDBConnection.ExecuteDataTable("SELECT * FROM " + this.SourceTableNames.SubgroupVals);
                DataTable DTIUS = this.SourceDBConnection.ExecuteDataTable("SELECT * FROM " + this.SourceTableNames.IndicatorUnitSubgroup);
                _FootNoteDataTable = new DataTable();
                _FootNoteDataTable.Columns.Add(FootNotes.FootNoteNId);
                _FootNoteDataTable.Columns.Add(FootNotes.FootNote);
                _FootNoteDataTable.Columns.Add(FootNotes.FootNoteGId);
                
                
                string[] DataNIdArray = dataNIds.Split(new string[] { "," }, StringSplitOptions.None);
                for (int i = 0; i < DataNIdArray.Length; i++)
                {
                    Sql = string.Empty;
                    Sql = "SELECT D.*, I." + Indicator.IndicatorGId + ", U." + Unit.UnitGId + ",IC." + IndicatorClassifications.ICName + ", A." + Area.AreaID + " FROM " + this.SourceTableNames.Data + " AS D, " + this.SourceTableNames.Indicator + " AS I, " + this.SourceTableNames.Unit + " AS U, " + this.SourceTableNames.IndicatorClassifications + " AS IC, " + this.SourceTableNames.Area + " AS A ";
                    Sql += "  WHERE D." + Data.IndicatorNId + "= I." + Indicator.IndicatorNId + " AND U." + Unit.UnitNId + " = D." + Data.UnitNId +
                        " AND IC." + IndicatorClassifications.ICNId + " = D." + Data.SourceNId +
                        " AND D." + Data.AreaNId + " = A." + Area.AreaNId + " AND D." + Data.DataNId + " ='" + DataNIdArray[i].ToString() + "'";
                    DataTable dtData = this.SourceDBConnection.ExecuteDataTable(Sql);
                    if (dtData.Rows.Count > 0)
                    {
                        DataRow dtNewRow = dtData.Rows[0];
                        string subgroup = string.Empty;
                        DataRow[] SubGroupValNidRow = null;
                        string SubgroupVal_NID = string.Empty;
                        DataRow[] IUSRow = null;
                        string IUSNIdGenerated = string.Empty;
                        int DataNId = 0;
                        //Get SubgroupValNId
                        if (dtNewRow["SG_Total"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Total", DataNId);
                        }
                        if (dtNewRow["SG_Male"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Male", DataNId);

                        } if (dtNewRow["SG_Female"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Female", DataNId);

                        } if (dtNewRow["SG_Urban"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Urban", DataNId);

                        } if (dtNewRow["SG_Rural"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Rural", DataNId);
                        } if (dtNewRow["SG_Poorest"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Poorest", DataNId);

                        } if (dtNewRow["SG_Second"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Second", DataNId);
                        } if (dtNewRow["SG_Middle"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Middle", DataNId);

                        }
                        if (dtNewRow["SG_Fourth"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Fourth", DataNId);

                        }
                        if (dtNewRow["SG_Richest"].ToString().Trim() != string.Empty)
                        {
                            DataNId = DataNId + 1;
                            GenerateNewUT_Data(DtSourceData, DTSubgroupVal, DTIUS, dtNewRow, ref subgroup, ref SubGroupValNidRow, ref SubgroupVal_NID, ref IUSRow, "Richest", DataNId);
                        }
                    }
                }
        ...
        private static void GenerateNewUT_Data(DataTable dtPublishData, DataTable DTSubgroupVal, DataTable DTIUS, DataRow dtNewRow, ref string subgroup, ref DataRow[] SubGroupValNidRow, ref string SubgroupVal_NID, ref DataRow[] IUSRow, string subgroupString,int dataNId)
        {
            subgroup = string.Empty;
            SubGroupValNidRow = null;
            SubGroupValNidRow = null;
            IUSRow = null;
            string Subgroup_Val_GId = string.Empty;
            if (Convert.ToBoolean(dtNewRow["Is_Inconsistent_AgeGroup"].ToString()) == true)
            {
                if (dtNewRow["InconsistentAgeGroup"].ToString() == string.Empty)
                {
                    subgroup = subgroupString;
                }
                else
                {
                    subgroup = subgroupString + " " + dtNewRow["InconsistentAgeGroup"].ToString();
                }
            }
            else
            {
                if (dtNewRow["AgeGroup"].ToString() == string.Empty)
                {
                    subgroup = subgroupString;
                }
                else
                {
                    subgroup = subgroupString + " " + dtNewRow["AgeGroup"].ToString();
                }
            }
            if (subgroup != string.Empty)
            {
                SubGroupValNidRow = DTSubgroupVal.Select(SubgroupVals.SubgroupVal + " = '" + subgroup + "'");
                if (SubGroupValNidRow != null)
                {
                    if (SubGroupValNidRow.Length > 0)
                    {
                        SubgroupVal_NID = SubGroupValNidRow[0][SubgroupVals.SubgroupValNId].ToString();
                        Subgroup_Val_GId = SubGroupValNidRow[0][SubgroupVals.SubgroupValGId].ToString();
                        if (SubgroupVal_NID != string.Empty)
                        {
                            //Get IUSNID
                            IUSRow = DTIUS.Select(Indicator.IndicatorNId + " = '" + dtNewRow[Indicator_Unit_Subgroup.IndicatorNId].ToString() + "' AND " + Indicator_Unit_Subgroup.UnitNId + " = '" + dtNewRow[Unit.UnitNId].ToString() + "' AND " + Indicator_Unit_Subgroup.SubgroupValNId + " = '" + SubgroupVal_NID + "'");
                            if (IUSRow != null)
                            {
                                if (IUSRow.Length > 0)
                                {
                                    //Insert Record
                                    DataRow newRow = dtPublishData.NewRow();
                                    newRow[0] = dataNId;
                                    newRow[1] = IUSRow[0][Indicator_Unit_Subgroup.IUSNId].ToString();
                                    newRow[2] = dtNewRow[Timeperiods.TimePeriodNId].ToString();
                                    newRow[3] = dtNewRow[Area.AreaNId].ToString();
                                    newRow[4] = dtNewRow["SG_" + subgroupString].ToString();
                                    newRow[5] = DateTime.Now;
                                    newRow[6] = DateTime.Now;
                                    newRow[7] = "0";
                                    if (dtNewRow["SG_" + subgroupString + "_FN"].ToString().Trim() == string.Empty)
                                    {
                                        newRow[8] = "-1";    
                                    }
                                    else
                                    {
                                        _FootNoteNId = _FootNoteNId + 1;
                                        DataRow FNRow = _FootNoteDataTable.NewRow();
                                        FNRow[0] = _FootNoteNId;
                                        FNRow[1] = dtNewRow["SG_" + subgroupString + "_FN"].ToString();
                                        FNRow[2] = Guid.NewGuid();
                                        _FootNoteDataTable.Rows.Add(FNRow);
                                        newRow[8] = _FootNoteNId;
                                        //Insert FootNote
                                    }    
                                    newRow[9] = dtNewRow["Source_NId"].ToString();
                                    newRow[10] = dtNewRow[Indicator.IndicatorNId].ToString();
                                    newRow[11] = dtNewRow[Unit.UnitNId].ToString();
                                    newRow[12] = SubgroupVal_NID;
                                    newRow[13] = "-1";
                                    newRow[14] = string.Empty;
                                    newRow[15] = dtNewRow["Status"].ToString();
                                    newRow[16] = dtNewRow["Add_Username"].ToString();
                                    newRow[17] = dtNewRow["Add_Date"].ToString();
                                    newRow[18] = dtNewRow["Add_UserRole"].ToString();
                                    newRow[19] = dtNewRow["Modified_Username"].ToString();
                                    newRow[20] = dtNewRow["Modified_Date"].ToString();
                                    newRow[21] = dtNewRow["Is_Inconsistent_AgeGroup"].ToString();
                                    newRow[22] = dtNewRow["PageTableNo"].ToString();
                                    newRow[23] = dtNewRow["Modified_Date"].ToString();
                                    newRow[24] = dtNewRow[Timeperiods.TimePeriod].ToString();
                                    newRow[25] = false;
                                    newRow[26] = false;
                                    newRow[27] = false;
                                    newRow[28] = dtNewRow[Indicator.IndicatorGId].ToString();
                                    newRow[29] = dtNewRow[Unit.UnitGId].ToString();
                                    newRow[30] = Subgroup_Val_GId;
                                    newRow[31] = dtNewRow[IndicatorClassifications.ICName].ToString();
                                    newRow[32] = dtNewRow[Area.AreaID].ToString();
                                    newRow[33] = dtNewRow[Timeperiods.TimePeriod].ToString();
                                    dtPublishData.Rows.Add(newRow);
                                }
                            }
                        }
                    }
        ...
        private void InsertFootNotes()
        {
            ...
            {
                string Sql = string.Empty;
                foreach (DataRow FNRow in _FootNoteDataTable.Rows)
                {
                    Sql = "Insert INTO " + this.DBTableNames.FootNote + " VALUES('" + FNRow[0].ToString() + "','" + FNRow[1].ToString() + "','" + FNRow[2].ToString() + "')";
                    this.DestDBConnection.ExecuteNonQuery(Sql);
                }
        ...
        internal ExportOnlineDatabaseCRING(UserSelection userSelection, DIConnection sourceDBConnection, DIQueries sourceDBQueries)
        {
        ...
        private static DataTable _FootNoteDataTable;
        private static int _FootNoteNId = 0;

        #endregion

        #region "Methods"

        internal bool ExportMDB(string destinationDBNameWPath, string tempFolderPath, bool createNewDatabase, string dataNIds)
        {
            bool RetVal = false;
            int ProgressCount = 0;
            try
            {
                ...
                {
                    this.ExportMDB(destinationDBNameWPath, tempFolderPath, dataNIds);
                }
                else
                {
        ...
        internal bool ExportMDB(string destinationDBNameWPath, string tempFolderPath,string dataNIds)
        {
            bool RetVal = false;
