...
--CREATE STORED PROCEDURE : sp_UpdateISMRD
...
BEGIN TRY
IF EXISTS (
SELECT * FROM sysobjects WHERE Name = ''TempTable''
)
DROP TABLE TempTable
END TRY
BEGIN CATCH
END CATCH
SELECT MRDTable.*, T2.Timeperiod_nid INTO  TempTable FROM  
( 
SELECT  d.IUSNId, d.Area_NId, MAX(t.TimePeriod) AS timeperiod  
FROM UT_Data  d,UT_Timeperiod t 
WHERE d.TimePeriod_NId= t.TimePeriod_NId 
GROUP BY d.IUSNId,d.Area_NId
) 
AS MRDTable , UT_timeperiod T2 
WHERE MRDTable.timeperiod=T2.Timeperiod

UPDATE  d
SET d.IsMRD=1, d.MultipleSource=1
FROM UT_Data AS d
INNER JOIN  TempTable AS t ON (
d.Timeperiod_nid = t.TimePeriod_NId) 
AND (d.Area_NId = t.Area_NId) 
AND (d.IUSNId = t.IUSNId)
...
BEGIN
EXEC dbo.sp_executesql @statement = N'
ALTER Procedure [dbo].[sp_UpdateISMRD]
AS
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_GET_ALL_LANGUAGE_CODES_NAMES]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'

CREATE PROCEDURE SP_GET_ALL_LANGUAGE_CODES_NAMES
AS
BEGIN
	SELECT LANGUAGE_CODE,REPLACE(Language_Name,''DI_'','''') AS Language_Name  FROM UT_LANGUAGE
END
...
ALTER PROCEDURE SP_GET_ALL_LANGUAGE_CODES_NAMES
AS
BEGIN
	SELECT LANGUAGE_CODE,REPLACE(Language_Name,''DI_'','''') AS Language_Name  FROM UT_LANGUAGE
END
...
Subgroup_Val as Subgroup,Area_Name as Area,area_id as ''Area ID'',
