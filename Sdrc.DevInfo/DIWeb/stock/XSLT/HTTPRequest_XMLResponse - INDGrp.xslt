  ...
  <body>
    ...
    <table style="font:12px Helvetica, sans-serif; color:#404141;border:1px solid #c8c8c8;" cellpadding="0" cellspacing="0" width="100%">
      <tr style="padding:7px 5px; font-size:14px;background:#eee" align="left">
        <th width="17%" style="border:1px solid #c8c8c8;border-right:0px;border-left:0px;border-top:0px;">Area</th>
	...
	<xsl:for-each select="Root/Data/Observation">
		...
		<xsl:if test="not(INDICATOR = preceding-sibling::Observation[1]/INDICATOR)">
			<tr style="padding:15px 0px 15px 2px;margin-left:auto;margin-right:auto;">
			<td style = "border:1px solid #c8c8c8;border-right:0px;border-top:0px;border-left:0px;" colspan="4">
			<font style="font:bold 14px"><xsl:value-of select="INDICATOR"/></font> (<xsl:value-of select="UNIT"/>), <xsl:value-of select="SUBGROUP"/>
			</td></tr>
		</xsl:if>
		...
		<tr style="padding:5px 5px 13px;">
			...
			<!-- AREA -->
			<td style="border:1px solid #c8c8c8;border-right:0px;border-top:0px;border-left:0px;vertical-align:top">
        <A target="_new">
          <xsl:attribute name="href">
            <xsl:value-of select="$BaseUrl"></xsl:value-of><xsl:value-of select="$Rest"></xsl:value-of>
            <xsl:value-of select="INDICATOR"/>/<xsl:value-of select="AREA"/>
          </xsl:attribute>
	...
	<tr style="padding:15px 0px 15px 2px;border:0px;margin-left:auto;margin-right:auto;">
		<td align="center" colspan="5"><a target="_new" style="text-decoration:none;" title="Powered by DevInfo">
      <xsl:attribute name="href">
