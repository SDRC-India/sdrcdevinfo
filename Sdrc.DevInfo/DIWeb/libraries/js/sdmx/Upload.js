...
function onPageLoad(hdsby, hdbnid, hselarea, hselind, hlngcode, hlngcodedb, hselindo, hselareao, hLoggedInUserNId, hLoggedInUserName, hOriginaldbnid) {
    ...
    // ************************************************
    createFormHiddenInputs("frm_sdmxUpload", "POST");
    SetCommonLinksHref("frm_sdmxUpload", "POST");
    SetOriginaldbnidInForm("frm_sdmxUpload", hOriginaldbnid);
    ...
    SetCommonRegistryPageDetails('RegUpload.aspx', "frm_sdmxUpload", "POST");
}
            ...
            success: function (data) {
                try {
                    if (action == 'A') {
                        ...
                        if (AssosciatedDatabases.length > 0)
                            z("selAssosciatedDatabase").options[1].selected = "selected";
                    }
                    else {
                        ...
                        //    if (AssosciatedDatabases.length > 0)
                        //alert(AssosciatedDatabases.length);
                        // z("selAssosciatedDatabase").options[1].selected = "selected";
                    }
...
function ValidateUploadDSD(action, FileNameWPath) {
    ...
    var RetVal = true;
    var ArrStatusAndMessage = '';
    var StatusAndMessage = '';
    var Status = '';
    var Message = '';

    StatusAndMessage = CheckIfDSDAlreadyUploaded(FileNameWPath);
    ArrStatusAndMessage = StatusAndMessage.split('[****]');
    Status = ArrStatusAndMessage[0];
    Message = ArrStatusAndMessage[1];
    if (Status == "True") {
        Status = true;
    }
    else {
    ...
    if (Status) {//CheckIfDSDAlreadyUploaded(FileNameWPath)
        if (action == 'A')//Add
        {
...
function OpenHeaderUpdatePopup(IsDevInfoDatabase, DbNid) {
    ShowLoadingDiv();
    var DefaultDSDName = getDefaultDSDName(DbNid);
    try {
        SetCloseButtonInPopupDiv(di_jq('#DivHeaderPopUp'), 'HideHeaderUpdatePopup', '../../stock/themes/default/images/close.png');
        di_jq('#DivHeaderPopUp').show('slow');
        CreateSenderRecieverHeader(IsDevInfoDatabase, DbNid, DefaultDSDName);
        ApplyMaskingDiv(100);
        GetWindowCentered(z("DivHeaderPopUp"), 710, 350);
        di_jq('#DivHeaderPopUp').css('top', '105px');
        LoadLanguage(readCookie("hlngcode"));
    }
    catch (err) { }
}

function HideHeaderUpdatePopup() {
    RemoveMaskingDiv();
    di_jq('#DivHeaderPopUp').hide('slow');
}
