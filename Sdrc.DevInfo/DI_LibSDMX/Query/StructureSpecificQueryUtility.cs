...
{
    internal class StructureSpecificQueryUtility : BaseQueryUtility
    {
        #region "Properties"
        ...
        internal StructureSpecificQueryUtility(Dictionary<string, string> dictUserSelections, DataReturnDetailTypes dataReturnDetailType, string agencyId, DIConnection DIConnection, DIQueries DIQueries)
        ...
        internal override XmlDocument Get_Query()
        {
            ...
            ORItem = null;
            ANDItem = null;
            StructureSpecificDataQuery = new SDMXObjectModel.Message.DataQueryType();
            StructureSpecificDataQuery.Header = Get_Appropriate_Header();
            StructureSpecificDataQuery.Query = new SDMXObjectModel.Query.DataQueryType();
            StructureSpecificDataQuery.Query.ReturnDetails = new DataReturnDetailsType();
            StructureSpecificDataQuery.Query.ReturnDetails.detail = Enum.GetName(typeof(DataReturnDetailTypes), this.DataReturnDetailType).ToString();
            StructureSpecificDataQuery.Query.DataWhere = new DataParametersAndType();
            StructureSpecificDataQuery.Query.DataWhere.DataStructure = this.Get_DataStructure_Reference();
            StructureSpecificDataQuery.Query.DataWhere.Or = new List<DataParametersOrType>();
            ...
            if (this.DictUserSelections != null)
            {
                ...
                foreach (string Key in this.DictUserSelections.Keys)
                {
                    ...
                    StructureSpecificDataQuery.Query.DataWhere.Or.Add(ORItem);
                }
            }
            RetVal = Serializer.SerializeToXmlDocument(typeof(SDMXObjectModel.Message.DataQueryType), StructureSpecificDataQuery);
